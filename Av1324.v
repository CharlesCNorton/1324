...
Definition TRIPw (b : list nat) (n : nat) : nat :=
  fold_right (fun y acc => ((if safeb b y then TPG b n y else 0) + acc)%nat)
             0%nat (seq 1 n).

Definition BBwp (u : list nat) (M : nat) : nat :=
  fold_right (fun y acc => (BBin u M y + acc)%nat) 0%nat (seq 1 M).

Definition GGwp (u : list nat) (M : nat) : nat :=
  fold_right (fun y acc => (GGin u M y + acc)%nat) 0%nat (seq 1 M).

Lemma BBin_top : forall u M, BBin u M M = Bin u M M.
Proof.
  intros u M. unfold BBin. rewrite Hu_top.
  replace (M - M) with 0 by lia.
  replace (seq M 1) with (M :: nil) by reflexivity.
  cbn [fold_right]. lia.
Qed.

Lemma BBin_safe_right : forall b y,
  is_perm b (length b) -> safe_at b y ->
  BBin b (length b) y = TBin b (length b) y.
Proof.
  intros b y Hpb Hs. unfold BBin, TBin.
  rewrite (proj1 (safe_iff_Hu b y Hpb) Hs). reflexivity.
Qed.

Theorem BBw_midmax : forall a b,
  is_perm a (length a) -> is_perm b (length b) ->
...
Proof.
  ...
  { unfold Bin. rewrite Hu_top.
    ...
    cbn [fold_right]. rewrite Hu_top. lia. }
  assert (Plo : fold_right (fun y acc =>
      (BBin (midmax a b) (length a + S (length b)) y + acc)%nat) 0%nat
      (seq 1 (length b))
    = (BBwp b (length b) + (length a + 1) * TRIPw b (length b)
       + (length a * length b + Awp a (length a)
          + (length a + S (length b))) * pairge (safelist b (length b))
       + length (safelist b (length b))
         * (length b * Lsum a (length a) + Bwp a (length a)
  ...
  { transitivity (fold_right (fun y acc =>
        (BBin b (length b) y
         + (if safeb b y
            then ((length a + 1) * TPG b (length b) y)%nat else 0)
         + (if safeb b y
            then ((length a * length b + Awp a (length a)
                   + (length a + S (length b)))
                  * cntge (safelist b (length b)) y)%nat else 0)
         + (if safeb b y
            then (length b * Lsum a (length a) + Bwp a (length a)
                  + (length a + S (length b)))%nat else 0)
         + acc)%nat) 0%nat (seq 1 (length b))).
    - apply nfold_ext_in. intros y Hy. apply in_seq in Hy.
      destruct (safeb b y) eqn:Es.
      + assert (Hs : safe_at b y) by (apply safeb_spec; exact Es).
        rewrite (BBin_lo_safe a b y Hpa Hpb ltac:(lia) ltac:(lia) Hs).
        rewrite (BBin_safe_right b y Hpb Hs). lia.
      + assert (Hns : ~ safe_at b y).
        { intro C. assert (K : safeb b y = true) by (apply safeb_spec; exact C).
          rewrite Es in K. discriminate. }
        rewrite (BBin_lo_unsafe a b y Hpa Hpb ltac:(lia) ltac:(lia) Hns). lia.
    - rewrite (nfold_four nat (fun y => BBin b (length b) y)
                 (fun y => if safeb b y
                           then ((length a + 1) * TPG b (length b) y)%nat
                           else 0)
                 (fun y => if safeb b y
                           then ((length a * length b + Awp a (length a)
                                  + (length a + S (length b)))
                 ...
                 (fun y => if safeb b y
                           then (length b * Lsum a (length a)
                                 + Bwp a (length a)
                                 + (length a + S (length b)))%nat
                           else 0)
                 (seq 1 (length b))).
      cbn beta.
      assert (Q1 : fold_right (fun y acc =>
          ((if safeb b y
            then ((length a + 1) * TPG b (length b) y)%nat else 0) + acc)%nat)
          0%nat (seq 1 (length b))
        = ((length a + 1) * TRIPw b (length b))%nat).
      { unfold TRIPw.
        rewrite <- (nfold_scal nat (length a + 1)
                      (fun y => if safeb b y then TPG b (length b) y else 0)
                      (seq 1 (length b))).
        apply nfold_ext_in. intros y _. destruct (safeb b y); lia. }
      assert (Q2 : fold_right (fun y acc =>
          ((if safeb b y
            then ((length a * length b + Awp a (length a)
                   + (length a + S (length b)))
                  * cntge (safelist b (length b)) y)%nat else 0) + acc)%nat)
          0%nat (seq 1 (length b))
        = ((length a * length b + Awp a (length a)
            + (length a + S (length b))) * pairge (safelist b (length b)))%nat).
      { transitivity (fold_right (fun y acc =>
            ((length a * length b + Awp a (length a)
              + (length a + S (length b)))
             * (if safeb b y then cntge (safelist b (length b)) y else 0)
             + acc)%nat) 0%nat (seq 1 (length b))).
        - apply nfold_ext_in. intros y _. destruct (safeb b y); lia.
        - rewrite (nfold_scal nat (length a * length b + Awp a (length a)
                     + (length a + S (length b)))
                     (fun y => if safeb b y
                               then cntge (safelist b (length b)) y else 0)
                     (seq 1 (length b))).
          rewrite (nfold_filter nat (safeb b)
                     (fun y => cntge (safelist b (length b)) y)
                     (seq 1 (length b))).
          unfold pairge, safelist. reflexivity. }
      assert (Q3 : fold_right (fun y acc =>
          ((if safeb b y
            then (length b * Lsum a (length a) + Bwp a (length a)
                  + (length a + S (length b)))%nat else 0) + acc)%nat)
          0%nat (seq 1 (length b))
        = (length (safelist b (length b))
           * (length b * Lsum a (length a) + Bwp a (length a)
              + (length a + S (length b))))%nat).
      { rewrite (nfold_filter nat (safeb b)
                   (fun _ : nat => (length b * Lsum a (length a)
                                    + Bwp a (length a)
                                    + (length a + S (length b)))%nat)
                   (seq 1 (length b))).
        rewrite (nfold_const nat (length b * Lsum a (length a)
                   + Bwp a (length a) + (length a + S (length b)))
                   (filter (safeb b) (seq 1 (length b)))).
        unfold safelist. lia. }
      rewrite Q1, Q2, Q3. unfold BBwp. lia. }
  assert (Phi : fold_right (fun y acc =>
      (BBin (midmax a b) (length a + S (length b)) y + acc)%nat) 0%nat
      (seq (S (length b)) (length a))
    = (length b * (GGwp a (length a) + Lsum a (length a))
  ...
  { replace (seq (S (length b)) (length a))
      ...
      by (rewrite seq_add_map; reflexivity).
    rewrite (nfold_map_gen nat nat
               (BBin (midmax a b) (length a + S (length b)))
               (fun t => t + length b) (seq 1 (length a))).
    cbn beta.
    transitivity (fold_right (fun t acc =>
        (length b * (GGin a (length a) t + S (hgap a (length a) t))
         + BBin a (length a) t + acc)%nat) 0%nat (seq 1 (length a))).
    - apply nfold_ext_in. intros t Ht. apply in_seq in Ht.
      apply (BBin_hi a b t Hpa Hpb ltac:(lia) ltac:(lia)).
    - rewrite (fold_add_split nat
                 (fun t => (length b
                            * (GGin a (length a) t
                               + S (hgap a (length a) t)))%nat)
                 (fun t => BBin a (length a) t) (seq 1 (length a))).
      cbn beta.
      rewrite (nfold_scal nat (length b)
                 (fun t => (GGin a (length a) t
                            + S (hgap a (length a) t))%nat)
                 (seq 1 (length a))).
      rewrite (fold_add_split nat (fun t => GGin a (length a) t)
                 (fun t => S (hgap a (length a) t)) (seq 1 (length a))).
      cbn beta.
      rewrite (nfold_succ_gap a (length a)).
      unfold GGwp, BBwp. reflexivity. }
  unfold BBw. rewrite (seq_split_three (length a) (length b)).
  rewrite !(nfold_app nat (BBin (midmax a b) (length a + S (length b)))).
  replace (seq 0 1) with (0%nat :: @nil nat) by reflexivity.
  replace (seq (length a + S (length b)) 1)
    with ((length a + S (length b)) :: nil) by reflexivity.
  rewrite !(nfold_single nat (BBin (midmax a b) (length a + S (length b)))).
  rewrite (BBin_root (midmax a b) (length a + S (length b))).
...
Lemma conv_BB10_val : forall m,
  (2 * conv GGwptot (fun n => (n * card132 n)%nat) m + m * Aconv m
   = wsum (fun k => (S k * S k * (m - k))%nat) m + Bconv m)%nat.
Proof.
  intro m.
  assert (E : (conv (fun k => (2 * GGwptot k)%nat)
                 (fun n => (n * card132 n)%nat) m
               + conv (fun k => 4 ^ k) (fun n => (n * card132 n)%nat) m
               = conv (fun k => (S k * S k * card132 k)%nat)
                   (fun n => (n * card132 n)%nat) m)%nat).
  { rewrite <- (conv_add_l (fun k => (2 * GGwptot k)%nat)
                  (fun k => 4 ^ k) (fun n => (n * card132 n)%nat) m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := GGwptot_closed n).
    rewrite <- (card132_binom n) in K. nia. }
  rewrite (conv_scal_l 2 GGwptot (fun n => (n * card132 n)%nat) m) in E.
  rewrite (conv_wsum2 (fun k => (S k * S k)%nat) (fun n => n) m) in E.
  assert (R := conv_rbconv m). lia.
Qed.

(* the two Ltot blocks against the shifted class *)

Lemma conv_Ltot_cat : forall m,
  (2 * conv Ltot card132 m + wsum S m = Aconv m)%nat.
Proof.
  intro m.
  assert (E : (conv (fun k => (2 * Ltot k)%nat) card132 m
               + conv (fun k => (S k * card132 k)%nat) card132 m
               = conv (fun k => 4 ^ k) card132 m)%nat).
  { rewrite <- (conv_add_l (fun k => (2 * Ltot k)%nat)
                  (fun k => (S k * card132 k)%nat) card132 m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := Ltot_closed n).
    rewrite <- (card132_binom n) in K. lia. }
  rewrite (conv_scal_l 2 Ltot card132 m) in E.
  rewrite (conv_wsum S m) in E.
  rewrite (conv_aconv m) in E. lia.
Qed.

Lemma conv_Ltot_shift : forall m,
  (2 * conv Ltot (fun n => card132 (S n)) m + wsum S (S m) + 4 ^ (S m)
   = Aconv (S m) + S (S m) * card132 (S m))%nat.
Proof.
  intro m.
  assert (E : (conv (fun k => (2 * Ltot k)%nat) (fun n => card132 (S n)) m
               + conv (fun k => (S k * card132 k)%nat)
                   (fun n => card132 (S n)) m
               = conv (fun k => 4 ^ k) (fun n => card132 (S n)) m)%nat).
  { rewrite <- (conv_add_l (fun k => (2 * Ltot k)%nat)
                  (fun k => (S k * card132 k)%nat)
                  (fun n => card132 (S n)) m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := Ltot_closed n).
    rewrite <- (card132_binom n) in K. lia. }
  rewrite (conv_scal_l 2 Ltot (fun n => card132 (S n)) m) in E.
  assert (S1 := conv_shift1 S m). cbn beta in S1.
  assert (A1 := conv_a1 m).
  lia.
Qed.

Lemma conv_kLtot_cat : forall m,
  (2 * conv (fun k => (k * Ltot k)%nat) card132 m
   + wsum (fun k => (k * S k)%nat) m = Bconv m)%nat.
Proof.
  intro m.
  assert (E : (conv (fun k => (2 * (k * Ltot k))%nat) card132 m
               + conv (fun k => (k * S k * card132 k)%nat) card132 m
               = conv (fun k => (k * 4 ^ k)%nat) card132 m)%nat).
  { rewrite <- (conv_add_l (fun k => (2 * (k * Ltot k))%nat)
                  (fun k => (k * S k * card132 k)%nat) card132 m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := Ltot_closed n).
    rewrite <- (card132_binom n) in K. nia. }
  rewrite (conv_scal_l 2 (fun k => (k * Ltot k)%nat) card132 m) in E.
  rewrite (conv_wsum (fun k => (k * S k)%nat) m) in E.
  rewrite (conv_bconv m) in E. lia.
Qed.

Lemma Bconv_shift1 : forall m,
  (fold_right (fun k acc => (k * (4 ^ k * card132 (S (m - k))) + acc)%nat)
     0%nat (seq 0 (S m)) + S m * 4 ^ (S m))%nat
  = Bconv (S m).
Proof.
  intro m. unfold Bconv. rewrite (seq_snoc (S m) 0).
  rewrite (nfold_app nat (fun k => (k * (4 ^ k * card132 (S m - k)))%nat)).
  rewrite (nfold_single nat (fun k => (k * (4 ^ k * card132 (S m - k)))%nat)
             (0 + S m)).
  cbn beta. f_equal.
  - apply nfold_ext_in. intros k Hk. apply in_seq in Hk.
    replace (S m - k)%nat with (S (m - k)) by lia. reflexivity.
...
Lemma conv_kLtot_shift : forall m,
  (2 * conv (fun k => (k * Ltot k)%nat) (fun n => card132 (S n)) m
   + wsum (fun k => (k * S k)%nat) (S m) + S m * 4 ^ (S m)
   = Bconv (S m) + S m * S (S m) * card132 (S m))%nat.
Proof.
  intro m.
  assert (E : (conv (fun k => (2 * (k * Ltot k))%nat)
                 (fun n => card132 (S n)) m
               + conv (fun k => (k * S k * card132 k)%nat)
                   (fun n => card132 (S n)) m
               = conv (fun k => (k * 4 ^ k)%nat) (fun n => card132 (S n)) m)%nat).
  { rewrite <- (conv_add_l (fun k => (2 * (k * Ltot k))%nat)
                  (fun k => (k * S k * card132 k)%nat)
                  (fun n => card132 (S n)) m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := Ltot_closed n).
    rewrite <- (card132_binom n) in K. nia. }
  rewrite (conv_scal_l 2 (fun k => (k * Ltot k)%nat)
             (fun n => card132 (S n)) m) in E.
  assert (S2 := conv_shift1 (fun k => (k * S k)%nat) m). cbn beta in S2.
  assert (B1 : (conv (fun k => (k * 4 ^ k)%nat) (fun n => card132 (S n)) m
                + S m * 4 ^ (S m))%nat = Bconv (S m)).
  { rewrite <- (Bconv_shift1 m). f_equal.
    unfold conv. apply nfold_ext_in. intros k _. cbn beta. ring. }
  lia.
Qed.
...
Lemma conv_a2 : forall m,
  (conv (fun k => 4 ^ k) (fun n => card132 (S (S n))) m
   + 4 ^ (S m) + 4 ^ (S (S m)))%nat
  = Aconv (S (S m)).
Proof.
  intro m.
  rewrite <- (conv_a1 (S m)).
  assert (E : conv (fun k => 4 ^ k) (fun n => card132 (S n)) (S m)
            = (conv (fun k => 4 ^ k) (fun n => card132 (S (S n))) m
               + 4 ^ (S m))%nat).
  { unfold conv. rewrite (seq_snoc (S m) 0).
    rewrite (nfold_app nat (fun k => (4 ^ k * card132 (S (S m - k)))%nat)).
    rewrite (nfold_single nat (fun k => (4 ^ k * card132 (S (S m - k)))%nat)
               (0 + S m)).
    cbn beta. f_equal.
    - apply nfold_ext_in. intros k Hk. apply in_seq in Hk.
      replace (S m - k)%nat with (S (m - k)) by lia. reflexivity.
    - replace (0 + S m)%nat with (S m) by lia.
      rewrite Nat.sub_diag. change (card132 1) with 1%nat. ring. }
  rewrite E. ring.
Qed.

Definition BBCL (n : nat) : Prop :=
  (4 * BBwptot n + n * (S n * card132 n)
   = n * n * (S n * card132 n) + n * 4 ^ n)%nat.

Lemma conv_BB1 : forall m,
  (forall j, (j <= m)%nat -> BBCL j) ->
  (4 * conv card132 BBwptot m + wsum (fun k => (k * S k)%nat) m
   = wsum (fun k => (k * k * S k)%nat) m + Bconv m)%nat.
Proof.
  intros m IH.
  assert (E : (conv card132 (fun n => (4 * BBwptot n)%nat) m
               + conv card132 (fun n => (n * S n * card132 n)%nat) m
               = conv card132 (fun n => (n * n * S n * card132 n
                                         + n * 4 ^ n)%nat) m)%nat).
  { rewrite <- (conv_add_r card132 (fun n => (4 * BBwptot n)%nat)
                  (fun n => (n * S n * card132 n)%nat) m).
    apply conv_ext; [intros k _; reflexivity|].
    intros n Hn. assert (K := IH n Hn). unfold BBCL in K. nia. }
  rewrite (conv_scal_r 4 card132 BBwptot m) in E.
  rewrite (conv_catw (fun n => (n * S n)%nat) m) in E.
  rewrite (conv_add_r card132 (fun n => (n * n * S n * card132 n)%nat)
             (fun n => (n * 4 ^ n)%nat) m) in E.
  rewrite (conv_catw (fun n => (n * n * S n)%nat) m) in E.
  rewrite (conv_rev card132 (fun n => (n * 4 ^ n)%nat) m), (conv_bconv m) in E.
  lia.
Qed.

Lemma conv_BB12 : forall m,
  (forall j, (j <= m)%nat -> BBCL j) ->
  (4 * conv BBwptot card132 m + wsum (fun k => (k * S k)%nat) m
   = wsum (fun k => (k * k * S k)%nat) m + Bconv m)%nat.
Proof.
  intros m IH.
  assert (E : (conv (fun k => (4 * BBwptot k)%nat) card132 m
               + conv (fun k => (k * S k * card132 k)%nat) card132 m
               = conv (fun k => (k * k * S k * card132 k
                                 + k * 4 ^ k)%nat) card132 m)%nat).
  { rewrite <- (conv_add_l (fun k => (4 * BBwptot k)%nat)
                  (fun k => (k * S k * card132 k)%nat) card132 m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n Hn. assert (K := IH n Hn). unfold BBCL in K. nia. }
  rewrite (conv_scal_l 4 BBwptot card132 m) in E.
  rewrite (conv_wsum (fun k => (k * S k)%nat) m) in E.
  rewrite (conv_add_l (fun k => (k * k * S k * card132 k)%nat)
             (fun k => (k * 4 ^ k)%nat) card132 m) in E.
  rewrite (conv_wsum (fun k => (k * k * S k)%nat) m) in E.
  rewrite (conv_bconv m) in E.
  lia.
Qed.

Lemma conv_Bwptot_shift : forall m,
  (4 * conv Bwptot (fun n => card132 (S n)) m
...
Proof.
  intro m.
  assert (E : (conv (fun k => (4 * Bwptot k)%nat) (fun n => card132 (S n)) m
               + conv (fun k => (2 * 4 ^ k)%nat) (fun n => card132 (S n)) m
               = conv (fun k => (2 * (S k * S k * card132 k))%nat)
                   (fun n => card132 (S n)) m
                 + conv (fun k => (k * 4 ^ k)%nat)
                     (fun n => card132 (S n)) m)%nat).
  { rewrite <- (conv_add_l (fun k => (4 * Bwptot k)%nat)
                  (fun k => (2 * 4 ^ k)%nat) (fun n => card132 (S n)) m).
    rewrite <- (conv_add_l (fun k => (2 * (S k * S k * card132 k))%nat)
                  (fun k => (k * 4 ^ k)%nat) (fun n => card132 (S n)) m).
    apply conv_ext; [|intros n _; reflexivity].
    intros n _. assert (K := Bwptot_closed n).
    rewrite <- (card132_binom n) in K. nia. }
  rewrite (conv_scal_l 4 Bwptot (fun n => card132 (S n)) m) in E.
  rewrite (conv_scal_l 2 (fun k => 4 ^ k) (fun n => card132 (S n)) m) in E.
  rewrite (conv_scal_l 2 (fun k => (S k * S k * card132 k)%nat)
             (fun n => card132 (S n)) m) in E.
  assert (A1 := conv_a1 m).
  assert (S1 := conv_shift1 (fun k => (S k * S k)%nat) m). cbn beta in S1.
  assert (B1 : (conv (fun k => (k * 4 ^ k)%nat) (fun n => card132 (S n)) m
                + S m * 4 ^ (S m))%nat = Bconv (S m)).
  { rewrite <- (Bconv_shift1 m). f_equal.
    unfold conv. apply nfold_ext_in. intros k _. cbn beta. ring. }
